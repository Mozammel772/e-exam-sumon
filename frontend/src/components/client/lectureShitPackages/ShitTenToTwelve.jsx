...
import { useCreateALectureShitPackageMutation } from "../../../redux/api/slices/lectureShitPackages";

export default function ShitTenToTwelve() {
  ...
  const [createALectureShitPackage, { isLoading: leactureShitLoader }] =
    useCreateALectureShitPackageMutation();

  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      const res = await createALectureShitPackage(paymentInfo);
      if (res?.data) {
        Swal.fire({
          title:
  ...
  return (
    <div className="ms-[275px] me-[20px] mt-20 solaimanlipi pb-5">
      <div className="bg-[#bbfdd3] py-6 mt-6">
        <p className="text-center font-bold solaimanlipi text-[#15793a] text-5xl mt-6">
          লেকচার শিট প্রশ্ন প্যাকেজ: নবম থেকে দ্বাদশ শ্রেণী পর্যন্ত
        </p>
      </div>

      <div className="bg-gradient-to-r from-[#024645] to-[#22c55e] py-10 mt-5 rounded-2xl shadow-lg border border-[#22c55e] max-w-full mx-auto px-4">
        <p className="text-center text-3xl text-[#ffffff] font-semibold tracking-wider animate-pulse">
          🎉 অফার! অফার! অফার! 🎉
        </p>
        <p className="text-5xl text-[#ffffff] font-extrabold text-center mt-4 solaimanlipi drop-shadow-sm">
          মাত্র ৯৯ টাকায় এসএসসি মডেল প্রশ্ন!
        </p>
        <p className="text-center text-lg text-white font-light mt-3">
          সীমিত সময়ের জন্য — আজই সংগ্রহ করুন!
        </p>
      </div>

      <div className="bg-[#e6f4f1] border-l-4 border-r-4 border-[#22c55e] p-4 rounded-lg shadow-sm max-w-full mx-auto mt-6 mb-10">
        <p className="text-[#024645] text-2xl font-semibold flex items-center gap-2 text-center justify-center">
          📢 নোটিশ:{" "}
          <span className="font-normal text-xl text-[#024645]">
            পেমেন্ট করার পর অটোমেটিক প্রয়োজনীয় এক্সেস চলে যাবে আপনার ইমেইল এ।
          </span>
        </p>
      </div>

      <div className="bg-green-50 border border-green-200 rounded-lg p-6 text-center shadow-sm max-w-full mx-auto mt-5">
        {/* Header */}
